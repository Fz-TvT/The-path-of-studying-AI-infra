...
FP16 (半精度) / BF16 (Brain Floating Point)：占 2 字节。用于前向传播、反向传播的大部分矩阵运算。优势：显存占用减半，Tensor Core 计算速度提升 2-8 倍（取决于 GPU 架构）。劣势：数值范围小，容易下溢 (Underflow) 变成 0，或上溢 (Overflow) 变成 NaN。FP32 (单精度)：占 4 字节。用于维护主权重副本 (Master Weights) 和优化器状态。作用：保证参数更新的精度和稳定性，防止微小梯度丢失。
